...
- The users are guided through a set of personal identification and needs questions that enable the app to find the right organization for them.
...
2. Identifying their needs, based on a hierarchy of questions derived from the SDG definition (Dots diagram) by grouping different subsections into similar areas.
   ...
   - "companies_options_map" for Company/Organisations

3. Showing the list of relevant organizations for the user based on their selections.
   The system will filter the organization data based on:

   - Personal identification information - for example if the user selects age: 0-12 -> filter only for organizations relevant children, gender, etc...)
   - SDG Group derived based on the needs questionnaire

4. Display detailed information about the organizations. Relevant details (Organization in Senegal) of the organization selected by user will be shown on the screen.
...
4. You can now run your application in 2 ways
   1. On a local computer using a phone emulator by starting Android Studio and launching the virtual device configured
...
All data required by the application is currently stored inside the application in the ./Frontend/assets/jsonFiles folder. This includes both organisations data as well as data used for the hierarchy mapping of the needs questionnaire options and mapping to SDG Group. The application uses this hierarchy mapping to dynamically generate the diffent options for the users depending on thir selection.

All .json files used by the application were generated based on the original .csv files that can be found here: ./Frontend/assets/csv files. Users can easily maintenain the .csv files and when ready the files can easily be converted to .json format and the application will use the new data.
...
- Updating the organisation.csv file. The structure of the file should be the same as the old version, just an update of the data content.
...
- .json file in ./Frontend/assets/jsonFiles should be updated accordingly

Of course there is also the option to manually update the underlying organisation.json file, but we would recommend to update the organisation.csv file first and generate the json file. This is so that the golde source of the data remins in a redable format in .csv file.

That's it, the application will now use the new data for the organisations. Remember to test the changes before you go ahead with the update.
...
- companies_options_map.json - stores the needs options for the Company/Organisation selection

This data can be updated in the same way as the organisation by updating the right file depending on whether there are changes required for the needs questionnaire for either Individuals or Company or both:

- Update the required .csv files. The structure of the file should be the same as the old version, just an update of the data content.
- Generate new json files based on the updated .csv files. This can be easily done by using the steps defined under [Conversion to json file](#conversion-to-json-file) section.
- .json files in .Frontend/assets/jsonFiles should be updated accordingly

That's it, the application will now use the new configuration data to generate the needs questionnaire. Remember to test the changes before you go ahead with the update.
...
4. ./Fontend/assets/jsonFiles should be updated accordingly.
